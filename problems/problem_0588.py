...
class Problem0588(EulerProblem):
    """
    ...
    $(x^4+x^3+x^2+x+1)^k$ are called quintinomial coefficients. (quintus= Latin
    for fifth).   Consider the expansion of $(x^4+x^3+x^2+x+1)^3$:
    ...
    $(x^4+x^3+x^2+x+1)^k$. So $Q(3)=7$.   You are given $Q(10)=17$ and
    $Q(100)=35$.  Find  $\sum_{k=1}^{18}Q(10^k) $.
    """
    problem_id = 588
    simple_input = 0
    simple_output = 1
